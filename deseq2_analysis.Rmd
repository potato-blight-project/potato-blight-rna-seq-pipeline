...
create_venn_diagrams_grid(
  ...
  config = list(
    list(
      ...
      categories=c("Outside infected area", "Inside infected area")
    ),
    list(
...
heatmap_definition <- list(
  inf = list(
    name = 'Inf. vs Mock',
    comparisons = c('DEG.SMira.Inf', 'DEG.SShona.Inf', 'DEG.Duke.Inf'),
    suffixes = c('_Mira', '_Shona', '_Duke')
  ),
  outinf = list(
    name = 'OutInf. vs Mock',
    comparisons = c('DEG.SMira.Out', 'DEG.SShona.Out', 'DEG.Duke.Out'),
    suffixes = c('_Mira', '_Shona', '_Duke')
  ),
  # control = list(
...
constitutive_filter <- function(x) {
  dplyr::select(x, ends_with('mira_vs_duke')) %>% lapply(function(x) x > 0) %>% as.data.frame() %>% rowSums(na.rm = T) == 4 |
    dplyr::select(x, ends_with('mira_vs_duke')) %>% lapply(function(x) x < 0) %>% as.data.frame() %>% rowSums(na.rm = T) == 4 |
    dplyr::select(x, ends_with('shona_vs_duke')) %>% lapply(function(x) x > 0) %>% as.data.frame() %>% rowSums(na.rm = T) == 4 |
    dplyr::select(x, ends_with('shona_vs_duke')) %>% lapply(function(x) x < 0) %>% as.data.frame() %>% rowSums(na.rm = T) == 4
}
