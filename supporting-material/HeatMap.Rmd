...
Hmap_data <- read.table(pipe("pbpaste"), header = T, sep = "\t") #Data from excel
View(Hmap_data)
```
...
Hmap_data.M <- as.matrix(Hmap_data[,seq(4, 36, 2)]) #cell wall and receptors
...
heatmap.2(Hmap_data.M,
          ...
          key.par = list(cex=0.6),
                #controls row height
          na.color = "darkgrey") #c(2, 20)  #vector of length 2 with margins for col names and row names
          + title(main= list("Cell wall DEGs - Inf48-vs-Mock48", cex = 1.5), line = 1)
#p
#dev.off()
```
